...
import pymongo  # التأكد من وجود الاستيراد هنا لحل مشكلة الصورة الأخيرة

# --- ⚙️ الإعدادات والربط ---
TOKEN = "7948017595:AAFpATTA4rHa5ED3N9d_gYbPgeOWIGdNqH8"
# استخدام الرابط المباشر لتجاوز خطأ DNS
...
user_state = {} # لتتبع مراحل إضافة السؤال
...
def start_cmd(message):
    ...
    bot.send_message(message.chat.id, "💎 لوحة تحكم ياسر @Ya_79k", reply_markup=markup)
...
# --- 🔄 نظام الإضافة المتسلسل (طلبك يا ياسر) ---

def step_get_q(message):
    user_state[message.from_user.id]['q'] = message.text
    msg = bot.send_message(message.chat.id, "✅ تم حفظ السؤال. الآن **أرسل الإجابة الصحيحة:**", parse_mode="Markdown")
    bot.register_next_step_handler(msg, step_get_correct_ans)

def step_get_correct_ans(message):
    uid = message.from_user.id
    user_state[uid]['opts'].append(message.text) # أول إجابة هي الصحيحة
...
def step_get_wrong_ans(message):
    ...
    step_get_correct_ans(message) # العودة للأزرار
...
def save_everything(call):
    ...
    q_doc = {
        "section": sec, "q": data['q'], "a": data['opts'][0],
        "options": data['opts'], "t": int(t)
    }
    q_collection.insert_one(q_doc)
    
    # إرسال للتخزين والسجل
    bot.send_message(STORAGE_GROUP_ID, f"📦 تخزين: {data['q']} في {sec}")
    bot.send_message(LOGS_GROUP_ID, f"📑 سجل: أضاف @Ya_79k سؤالاً جديداً.")
...
def run(): server.run(host="0.0.0.0", port=int(os.environ.get("PORT", 10000)))
